<script lang="ts">
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      console.log(dt);
      let record = await pb.collection("users").update(data?.user?.id, dt);
      console.log(record);
  ...
  const checkUsername = async (event: any) => {
    ...
    try {
      let record = await pb
        .collection("check_username_existance")
...
{#if updateSuccess}
  <p class="text-xl font-thin text-green-600">
    Your Account Data Is Updated Successfully
  </p>
{/if}
{#if error}
  <ErrorComp {error} />
